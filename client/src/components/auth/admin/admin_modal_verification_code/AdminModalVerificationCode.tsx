...
const AdminModalVerificationCode: React.FC = () => {
  ...
  const emailAdminLoginState = useAppSelector((state) => state.user.email);
  const cellphoneAdminLoginState = useAppSelector(
    (state) => state.user.cellphone
  );
  ...
  useEffect(() => {
    if (!emailAdminLoginState) {
      setShowErrorMessage(true);
      setErrorMessage("¡Error al obtener el correo del administrador!");
    }
  }, [emailAdminLoginState]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      if (responseNextAuth?.status === 200) {
        ...
        await router.replace("/admin/all_transactions", {
          scroll: false,
        });
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            {maskEmail(emailAdminLoginState || "")}
          </h5>
